...
namespace RiotSharp
{
    class RateLimitedRequester : Requester
    {
        ...
        {
            handleLast10M(region);
            handleLast10S(region);
        }

        private void handleLast10M(Region region)
        {
        ...
        private void handleLast10S(Region region)
        {
            ...
            holdRequestLast10S(region);

            if ((DateTime.Now - firstRequestsInLastTenS[region]).TotalSeconds > 10) //reset check
            {
        ...
        private void incCounterLast10S(Region region)
        {
        ...
        private void initSetLast10S(Region region)
        {
            if (!firstRequestsInLastTenS.ContainsKey(region))
            {
                firstRequestsInLastTenS.Add(region, DateTime.Now); //first call
            }
        }
    }
}
